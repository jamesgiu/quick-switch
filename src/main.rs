...
fn find_matching_directories<'a>(directories_list: Vec<&'a str>, input_term: &'a str) -> Result<Vec<MatchingDirectory<'a>>, String> {
    ...
    // Now do frequency
    println!("{:?}", matching_directory_count);
    ...
    println!("Winner: {}, score: {}", final_dirs[0].directory_name, final_dirs[0].score);
...
fn main() {
    let input = Args::parse().directory;
    ...
    match bash_history_lines {
        Ok(string_contents) => {
            ...
            let final_dirs_res = find_matching_directories(directories, &input);
